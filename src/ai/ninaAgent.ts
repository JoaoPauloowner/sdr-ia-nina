...
import { getNinaSystemPrompt } from './prompt.js';
import { ninaToolsDefinitions, executeTool } from './tools.js';
...
export class NinaAgent {
  ...
  async generateOutboundMessage(req: OutboundMessageRequest): Promise<string> {
    ...
    const linkSala = details?.closer.link_sala_reuniao || 'https://meet.google.com/abc-nina-demo';
    ...
    // Se a API da Anthropic estiver configurada, usa o Claude Haiku
    if (this.anthropic) {
      try {
        const systemPrompt = getNinaSystemPrompt();
...
- Case de sucesso do setor: ${caseSnippet}`;
        ...
        console.warn('Fallback para geração nativa da Nina devido a erro na API:', err);
      }
    }

    // Fallback nativo determinístico e perfeitamente aderente ao tom da Nina
  ...
  async handleInboundMessage(req: InboundMessageRequest): Promise<InboundResponse> {
    ...
    // 2. Detecção de cancelamento ou reagendamento
    if (
...
export const ninaAgent = new NinaAgent();
